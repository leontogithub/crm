...
<%@ page import="com.zcw.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.zcw.crm.workbench.domain.Tran" %>
...
<%
	...
	for (int i = 0; i < stageList.size(); i++) {
		...
		String possibilty = pMap.get(stage);
...
<script type="text/javascript">
	...
	function changeIcon(stage,index1) {
		...
		//如果当前阶段的可能性为0，前7个都是黑o,后两个是红x和黑x
		...
		}else {  //当前阶段可能性不为0，前7个样式不确定，后两个都是黑x
			//遍历前7个
			for(var i=0;i<point;i++){
				//如果是当前阶段
				if (i == index){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//准备当前阶段的可能性
			String currentPossibilty = pMap.get(currentStage);

			//判断当前阶段
			//如果当前阶段可能性为0,前7个阶段是黑圈，后两个一个是黑，一个是红
			if ("0".equals(currentPossibilty)){

				for (int i=0;i<stageList.size();i++) {
					//取得每一遍历出来的阶段，根据阶段取其可能性
					DicValue dv = stageList.get(i);
					String listStage = dv.getValue();
					String listPossibilty = pMap.get(listStage);
					//判断如果遍历出来的阶段的可能性为0，说明是后两个
					if ("0".equals(listPossibilty)) {  //为0

						if (listStage.equals(currentStage)) {//当前阶段红，
							//红x
				%>
				...
				<%
					...
					}else {  //遍历出来的阶段可能性不为0，说明是前7个，一定是黑o
						//黑o
				%>
